...
export default function GardenNotification() {
  ...
  async function enableNotifications() {
    const appMode = isStandaloneMode();
    const requiresInstall = environment.platform === "ios" && !appMode;
    ...
    if (!supported) {
      setStatus("error");
      setMessage(
        `${environment.browserName} 当前没有提供完整的 Web Push。桌面端和 Android 建议使用最新版 Chrome 或 Edge；iPhone 请先添加到主屏幕后再开启。`
      );
      return;
    }
      ...
      {panelOpen && (
        <div className="garden-bell-panel" role="dialog" aria-label="花园信使">
          ...
          {isSubscribed ? (
            ...
            <>
              <h2>通知暂时关闭</h2>
              <p>{environment.browser === "safari" ? "Safari 无痕窗口与普通窗口的权限和网站数据彼此隔离。请使用普通窗口测试，并在 Safari 与 macOS 通知设置中允许 LMN516。" : "请在当前浏览器或系统设置中重新允许 LMN516 发送通知。"}</p>
            </>
          ) : needsInstall ? (
            <>
              <h2>
